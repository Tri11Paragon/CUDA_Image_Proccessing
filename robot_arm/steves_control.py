...
class UserInterface:
    ...
    def make_move(self, piece):
        ...
        x
        y
        while good_place == False:
            good_place = True
            x = random(0, 2)
            y = random(0, 2)
            for x in range(taken_places):
                if f"s{x}_{y}" == taken_places[x]:
                    good_place = False
            if good_place == True:
                taken_places.append(f"s{x}_{y}")
                self.piece_to_square(self, piece, x, y)

    def player_place(square_x, square_y):
        taken_places.append(f"s{square_x}_{square_y}")
    ...
    def setup_game(self, event=None):
        ...
        for x in range(2, 5):
            ...
            time.sleep(1)
            self.close_grip()
            self.set_to(static_positions[x])
            self.open_grip
    
    def move_made(self, x, y):
        self.player_place(x, y)
        self.make_move(current_cube)
        current_cube += 1
    ...
    def run_function(self, event=None):
        ...
        if parts[0] == "c":
            ...
            self.accept_piece()
        elif parts[0] == "setup":	#Sets up the game by taking pieces from user and putting them into their "homes"
            self.setup_game
        elif parts[0] == "play":	#prompts robot's response to player making their move. Parts 2 and 3 are the x and y coords
            self.move_made(parts[1], parts[2])
